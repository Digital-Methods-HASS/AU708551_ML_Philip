...
```{r faceted-homicides, eval = TRUE}
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity)) +
  geom_line() +
  facet_wrap(~ Entity, ncol = 2, nrow = 3) +
  labs(
    title = "Homicide rates in Western Europe by country",
    x = "Year",
    y = "Homicides per 100,000",
    color = "Country"
  ) +
...
ggplot(data = kings, aes(x = Midyear, y = Duration)) +
  ...
  geom_smooth() +
  labs(
    title = "Danish monarchs through time",
    x = "Midyear",
    y = "Duration"
  ) +
  theme_bw()
...
The visualisations show that homicide rates in Western Europe were much higher in the past than they are today. The general pattern is a decline over time, although the countries do not follow exactly the same path. This points to a long-term reduction in homicides. From that perspective, one could say that society has become less violent in everyday life.

Still, I do not think the graphs are enough to answer whether we are more "civilized" today. The question is too broad.

The data only deals with homicide, leaving out many other forms of violence (war, colonialism, domestic abuse etc.) The data also comes from historical records that are uneven, since naturally we have more accurate data for contemporary times. We can conclude that the visuals support a narrower point: they show a sharp decline in recorded homicide rates in Western Europe over the long term. That says something important about changes in social behaviour and development of institutions (e.g. state monopoly of violence) but it does not settle the larger question of whether modern society is more "civilized" in every sense.